...
st.markdown("<center style=\"font-size:24px;\">This is a Movie recommendation system developed by Shreyas Kulkarni,Parth Kulkarni and Sakshi Palve of BEcomp</center>",unsafe_allow_html=True)
...
if st.sidebar.button('Recommend'):
    ...
    for i in range(0,9):
        if i%2==0:
            col1.write(f"<b>{recommendations[i]}</b>",unsafe_allow_html=True)
            col1.image(posters[i])
            col1.write('{}...'.format(describe[i])[:300])
            col1.write("Genre:{}".format(genres[i][0]['name']))
        else:
            col2.write(f"<b>{recommendations[i]}</b>",unsafe_allow_html=True)
